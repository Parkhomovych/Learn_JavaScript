...
const account = {
    ...
    deposit(amount) { // поповненя
        if (amount <= 0) { // якщо менше рівне нуля
            return 'Error'
        } else { 
    ...
    withdraw(amount) { // зняття
        if (amount > this.balanse) { // якщо сума  більша за поточний баланс
            return "Недостатньо коштів"
        } else if(amount === 0 || amount < 0){ // якщо сума  рівна або менша нуля
            return "Error"
        } else {
            console.log('Операція успішна'); 
        }                                                                                                                                        //* 'withdraw' * //
    ...
    getTransactionDetails(id) {
        for (const transaction of this.transactions) {// перебираємо масив транзакції
         if (transaction.id === id) { // якщо масив має такий id
